...
class TrainingTest(absltest.TestCase):
  ...
  def test_train_one_task_eval_two_tasks(self):
    ...
    eval_task_name_1 = 'eval_1'
    eval_task_1 = training.EvalTask(
        _very_simple_data(),  # deliberately re-using training data
        [tl.L2Loss()],
        name=eval_task_name_1,
    )
    eval_task_name_2 = 'eval_2'
    eval_task_2 = training.EvalTask(
        _very_simple_data(),  # deliberately re-using training data
        [tl.L2Loss()],
        name=eval_task_name_2,
    )
    training_session = training.Loop(
        model,
        tasks=(task,),
    ...
    eval_tasks_names = [task.name for task in training_session.eval_tasks]
    self.assertCountEqual([eval_task_name_1, eval_task_name_2],
                          eval_tasks_names)

  def test_update_eval_task_metrics(self):
    """Creates EvalTask with empty metrics list, then updates metrics."""
    eval_task = training.EvalTask(_very_simple_data(), [])
    self.assertEmpty(eval_task.metric_names)
    eval_task.metrics = [tl.L2Loss()]
    self.assertLen(eval_task.metric_names, 1)
    with self.assertRaises(ValueError):
      eval_task.metric_names = ['metric_1', 'metric_2']
